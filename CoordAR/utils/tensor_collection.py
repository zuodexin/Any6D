...
class TensorCollection:
    # 可以理解为一个字典，key是tensor的名字，value是tensor
    # 相比于字典，多了一些方法，比如to, cuda, cpu, float, double, half, clone
    # 还能往里面添加记录
    ...
    @staticmethod
    def stack(tcs: List["TensorCollection"], dim=0):
        tensors = dict()
        for k in tcs[0].tensors.keys():
            tensors[k] = torch.stack([tc.tensors[k] for tc in tcs], dim=dim)
        return TensorCollection(**tensors)

    def to_bytes(self):
        buffer = BytesIO()
        np_arrays = {k: v.cpu().numpy() for k, v in self.tensors.items()}
...
def to_device(
    src_data: Union[Dict[Any, Any], List[Any], torch.Tensor], device: str
) -> Union[Dict[Any, Any], List[Any], torch.Tensor]:
