...
def build_scheduler(lr_config, optimizer, epoch_length):
    ...
    if policy == "flat_and_anneal":
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, lambda it: it * alpha + beta
        )
    elif policy == "step":
        if len(lr_config["steps"]) != 1:
            raise ValueError(
                "step policy only support 1 step. got {}".format(
                    len(lr_config["steps"])
                )
            )
        step_size = lr_config["steps"][0] * total_iters  # by batch/iter
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size, lr_config["step_gamma"]
        )
    elif policy == "poly":
        # if update_mode == "batch":
        #     count = epoch_length * lr_config["epochs"]
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones, lr_config["step_gamma"]
        )
    else:
        raise ValueError(
            "Unrecognized scheduler type {}, "
            "valid options: 'flat_and_anneal', 'linear', 'step', 'poly', 'multistep'".format(
                policy
            )
        )
    return scheduler
...
def flat_and_anneal_lr_scheduler(
    ...
    if warmup_method not in ("constant", "linear", "pow", "exp"):
        raise ValueError(
            "Only 'constant', 'linear', 'pow' or 'exp' warmup_method accepted,"
            "got {}".format(warmup_method)
        )
        ...
        if x < anneal_start:
            ...
            if anneal_method == "step":
                ...
                lr_factor = target_lr_factor + 0.5 * (1 - target_lr_factor) * (
                    1
                    + cos(
                        pi * ((float(x) - anneal_start) / (total_iters - anneal_start))
                    )
                )
...
def flat_and_anneal_lr_scheduler_epoch(
    optimizer,
    total_epochs,
    warmup_epochs=0,
    warmup_factor=0.1,
    ...
    if anneal_method == "step":
        if any([_step < warmup_epochs / total_epochs or _step > 1 for _step in steps]):
            raise ValueError(
                "error in steps: {}. warmup_epochs: {} total_epochs: {}."
                "steps should be in ({},1)".format(
                    steps,
                    warmup_epochs,
                    total_epochs,
        ...
        if list(steps) != sorted(steps):
            raise ValueError("steps {} is not in ascending order.".format(steps))
        warnings.warn("ignore anneal_point when using step anneal_method")
        anneal_start = steps[0] * total_epochs
    else:
        if anneal_point > 1 or anneal_point < 0:
            raise ValueError(
                "anneal_point should be in [0,1], got {}".format(anneal_point)
            )
        anneal_start = anneal_point * total_epochs
    ...
    def f(x):  # x is the iter in lr scheduler, return the lr_factor
        ...
        if x < anneal_start:
            return 1
        elif x >= anneal_start and x < total_epochs:
            if anneal_method == "step":
                # ignore anneal_point and target_lr_factor
                milestones = [_step * total_epochs for _step in steps]
                lr_factor = step_gamma ** bisect_right(milestones, float(x))
            elif anneal_method == "cosine":
                # slow --> fast --> slow
                lr_factor = target_lr_factor + 0.5 * (1 - target_lr_factor) * (
                    1
                    + cos(
                        pi * ((float(x) - anneal_start) / (total_epochs - anneal_start))
                    )
                )
            elif anneal_method == "linear":
                # (y-m) / (B-x) = (1-m) / (B-A)
                lr_factor = target_lr_factor + (1 - target_lr_factor) * (
                    total_epochs - float(x)
                ) / (total_epochs - anneal_start)
            elif anneal_method == "poly":
                # slow --> fast if poly_power < 1
                # fast --> slow if poly_power > 1
                # when poly_power == 1.0, it is the same with linear
                lr_factor = (
                    target_lr_factor
                    + (1 - target_lr_factor)
                    * ((total_epochs - float(x)) / (total_epochs - anneal_start))
                    ** poly_power
                )
            elif anneal_method == "exp":
                # fast --> slow
                ...
                _target_lr_factor = max(target_lr_factor, 5e-3)
                lr_factor = _target_lr_factor ** (
                    (float(x) - anneal_start) / (total_epochs - anneal_start)
                )
            else:
                lr_factor = 1
            return lr_factor
...
def step_scheduler_with_warmup(optimizer, step_size, gamma, warmup_length):
    def lr_lambda(iter):
        if iter < warmup_length:
            return (iter + 1) / warmup_length
        else:
            return gamma ** ((iter - warmup_length) // step_size)
...
def test_flat_and_anneal():
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        )  # only get the first lr (maybe a group of lrs)
        for batch in range(epoch_len):
            # if global_step < lr_config['warmup_iters']:
            ...
            if global_step == 0 or (len(lrs) >= 1 and cur_lr != lrs[-1]):
                print(
                    "epoch {}, batch: {}, global_step:{} lr: {}".format(
