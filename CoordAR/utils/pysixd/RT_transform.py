...
def allocentric_to_egocentric(allo_pose, src_type="mat", dst_type="mat"):
    ...
    if angle > 0:
        ...
        if src_type == "mat" and dst_type == "quat":
            ...
            ego_pose = allo_pose.copy()[:3, :]
    return ego_pose


def allocentric_to_egocentric_batch_torch(allo_pose, src_type="mat", dst_type="mat"):
    # allo_pose: (N, 4, 4) or (N, 7)
    bs = allo_pose.shape[0]
    ego_pose = np.eye(4)[None].repeat(bs, 0)
    for i in range(bs):
        ego_pose[i, :3, :] = allocentric_to_egocentric(
            allo_pose[i].detach().cpu().numpy(), src_type=src_type, dst_type=dst_type
        )
    ego_pose = torch.from_numpy(ego_pose).to(allo_pose)
    return allo_pose
...
_MAX_FLOAT = np.finfo(np.float32).dtype
