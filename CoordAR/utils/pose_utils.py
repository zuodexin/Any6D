...
def rot_from_axangle_chain(ax_angles):
    ...
    return np.linalg.multi_dot(
        [axangle2mat(_axa[:3], np.pi * _axa[3]) for _axa in ax_angles]
    )
...
def inverse_warp(
    img, depth, pose, intrinsics, rotation_mode="euler", padding_mode="zeros"
):
...
def get_closest_rot_batch(pred_rots, gt_rots, sym_infos):
    ...
    sym_indices = torch.zeros(batch_size).long().to(device)  # B

    for i in range(batch_size):
        closest_rot, sym_idx = get_closest_rot(
...
def get_closest_pose_batch(poses_est, poses_gt, sym_infos):
    ...
    closest_poses_gt = torch.tensor(
        closest_poses_gt, device=device, dtype=poses_gt.dtype
    )
...
def get_closest_pose_batch_cpu(poses_est, poses_gt, sym_infos):
    ...
    for i in range(batch_size):
        closest_rot = get_closest_rot(
            rots_est[i], rots_gt[i], sym_infos[int(labels[i])]
        )
        closest_poses_gt[i][:4] = mat2quat(closest_rot)
    return closest_poses_gt
...
def T_transform_batch(T_src, T_delta, zoom_factor, labels_pred=None):
    ...
    if T_delta.shape[1] > 3:  # class aware
        assert (
            labels_pred is not None
        ), "labels_pred should not be None when class aware"
...
def se3_mul_torch_batch(RT1, RT2):
    ...
    T_new = torch.matmul(RT1[:, :3, :3], RT2[:, :3, 3].view(B, 3, 1)) + RT1[
        :, :3, 3
    ].view(B, 3, 1)
...
def calc_se3_torch_batch(poses_src, poses_tgt):
    """Bx3x4."""
    # B = poses_src.shape[0]
    se3_src2tgt_batch = se3_mul_torch_batch(
        poses_tgt, se3_inverse_torch_batch(poses_src)
    )
...
def test_euler2quat():
    ...
    t1 = time.perf_counter()
    for _ in range(runs):
        quat_from_euler_torch = euler2quat_torch(
            euler_torch[:, 0], euler_torch[:, 1], euler_torch[:, 2]
        )
    ...
    for _ in range(runs):
        for i in range(B):
            quat_from_euler_np[i].copy_(
                torch.tensor(euler2quat(euler_np[i, 0], euler_np[i, 1], euler_np[i, 2]))
            )
...
def test_qrot_points():
    ...
    models_cad_files = [
        osp.join(data_root, "models/obj_{:06d}.ply".format(i)) for i in range(1, 15 + 1)
    ]
    obj_id = 0
...
if __name__ == "__main__":
    ...
    transforms_sym = get_symmetry_transformations(
        models_info[cls_idx], max_sym_disc_step=0.01
    )
    # sym_info = axangle2mat([0, 0, 1], pi)
